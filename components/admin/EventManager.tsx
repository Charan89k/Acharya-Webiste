...
export function EventManager({ onUpdate }: EventManagerProps) {
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (editingId) {
      const success = await updateEvent({
        id: editingId,
        ...formValues,
        attendees: events.find((item) => item.id === editingId)?.attendees ?? 0,
      })
      if (success) {
        ...
        toast({ title: 'Update failed', description: 'Could not update event details.' })
      }
    } else {
      ...
      if (created) {
        ...
        toast({ title: 'Creation failed', description: 'Could not save the new event.' })
      }
    }
  }
  ...
  const handleDelete = async (eventId: string) => {
    ...
    if (success) {
      ...
      toast({ title: 'Delete failed', description: 'Could not delete the event.' })
    }
  }
  ...
  const handleFileUpload = async (file: File) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setFormValues((current) => ({ ...current, coverImage: data.url }))
        toast({ title: 'Uploaded', description: 'Banner image uploaded successfully.' })
      } else {
        toast({ title: 'Upload failed', description: 'Server rejected file upload.' })
      }
    } catch {
      toast({ title: 'Upload error', description: 'An error occurred during image upload.' })
    } finally {
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            <select
              ...
              className="mt-3 w-full rounded-md border border-slate-700 bg-slate-950/90 px-3 py-2 text-slate-100 animate-none focus:outline-none focus:ring-1 focus:ring-teal-300"
            >
          ...
          <div>
            ...
            <select
              ...
              className="mt-3 w-full rounded-md border border-slate-700 bg-slate-950/90 px-3 py-2 text-slate-100 focus:outline-none focus:ring-1 focus:ring-teal-300"
            >
          ...
          <div>
            <label className="text-sm font-medium text-slate-300">Upload Banner</label>
            ...
            {isUploading && <p className="text-xs text-teal-300 mt-1">Uploading image file...</p>}
          </div>
          ...
          {editingId && (
            <Button type="button" variant="outline" className="w-full rounded-full text-slate-200" onClick={resetForm}>
              Cancel editing
            </Button>
          )}
        </div>
          ...
          <div className="rounded-[1.75rem] border border-white/10 bg-slate-950/80 p-5">
            ...
            {formValues.coverImage ? (
              ...
              <div className="mt-4 flex min-h-[200px] items-center justify-center rounded-3xl border border-dashed border-white/10 bg-white/5 text-slate-500">
                Upload a banner to preview it here.
              </div>
            )}
          </div>
          ...
          <div className="rounded-[1.75rem] border border-white/10 bg-slate-950/80 p-5">
            ...
            <div className="mt-4 grid gap-4 sm:grid-cols-2">
              ...
              <select
                ...
                className="rounded-md border border-slate-700 bg-slate-950/90 px-3 py-2 text-slate-100 focus:outline-none"
              >
            ...
            {filteredEvents.map((eventData) => (
              <tr key={eventData.id} className="hover:bg-white/5 transition-colors duration-200">
                ...
                <td className="px-6 py-4 space-x-2">
                  <Button size="sm" variant="outline" className="rounded-full text-slate-200" onClick={() => handleEdit(eventData)}>
                    Edit
                  </Button>
